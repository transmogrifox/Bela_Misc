...
bool setup(BelaContext *context, void *userData)
{
    ...
    // Create the overdrive : make_overdrive() initializes sane (usable) default settings
    ...
    // The second call below is all that is needed after make_overdrive(), since the constructor function
    // sets bypass state to true.  A call to od_set_bypass(od, false) would then toggle to active-state
...
void render(BelaContext *context, void *userData)
{
    ...
    for(unsigned int n = 0; n < context->audioFrames; n++)
    {
        // Write overdrive through ch0, pass-through ch1
