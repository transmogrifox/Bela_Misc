...
void
compute_filter_coeffs_1p(iir_1p* cf, unsigned int type, float fs, float f0)
{
...
void
iir_get_response(iir_1p* cf, float n, float fstart, float fstop, float* frq, float* mag, float* phase)
{
    //printf"b0 = %f, b1 = %f, b2 = %f, a0 = %f, a1 = %f, a2 = %f, ");
    const int RE = 0;
    const int IM = 1;
    ...
    for(int i = 0; i < n; i++)
    {
        ...
        phase[i] = 180.0*(atanf(num[IM]/num[RE]) - atanf(den[IM]/den[RE]))/M_PI; // TODO: Implement phase calculation
...
s_biquad_to_z_biquad(float sgain, float fs_, float kz_, float* num, float* den)
{
    ...
    //printf("z-Biquad:\nsgain: %f, fs: %f, k: %f\nnum[0] = %f, num[1] = %f, num[2] = %f, den[0] = %f, den[1] = %f, den[2] = %f\n", sgain, fs_, kz_, num[0], num[1], num[2], den[0], den[1], den[2]);

}

void
s_biquad_bandpass_coeffs(float w0, float Q, float* num, float* den)
{
    num[0] = 0.0;
    num[1] = w0/Q;
    num[2] = 0.0;
    ...
    den[1] = w0/Q;
    den[2] = 1.0;
    //printf("s-Biquad:\nw0 = %f, Q = %f, num[1] = %f, den[0] = %f, den[1] = %f\n", w0, Q, num[1], den[0], den[1]);
}
